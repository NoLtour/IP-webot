...
class WheelEncoderCalculator: 
    ...
    def updatePose(this, worldPose:CartesianPose, leftRotation, rightRotation):
        """Takes input pose and applys motion of left and right rotation to get new position, which is applied to the input pose"""
        # Intially in local coordiantes
        movementPose = this.calculateProgression(leftRotation, rightRotation)
        
        # Rotated into world coordiantes
        movementPose.rotatePose( worldPose.roll, worldPose.pitch, worldPose.yaw )
        
        # Added onto this world pose
        worldPose.addPose( movementPose )
...
class Navigator:
    ...
    currentPose: CartesianPose
    wheelEncoderCalculator: WheelEncoderCalculator
    
    def __init__(this, wheelDiameter, wheelSeperation, startingPose=CartesianPose( 0, 0, 0, 0, 0, 0 ) ):
        this.targetNodes = []
    ...
    def desiredTargetMVels(this):
        ...
        targetDx = this.currentTarget.x - this.currentPose.x
        targetDy = this.currentTarget.y - this.currentPose.y
        ...
        targetDYaw = fixRads( np.arctan2( targetDy, targetDx ) - this.currentPose.yaw ) 
